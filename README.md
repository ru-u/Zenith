...
short thesis generated by an in-house quant engine. Built for DECA stock-market-game students
...
- **The pre-close drop** — ~30 minutes before the close, Zenith generates a short thesis for the
  top 5 movers and emails every user who hasn't opted out (Pro gets the full theses; free gets a
  teaser). That timing is the whole point: in the DECA game a trade entered during market hours
  fills at *that day's close*, so a thesis is only actionable if it lands before 4 PM ET.
...
| Pre-close email drop | teaser | ✅ full theses |
...
> pipeline is free and runs regardless — theses generate and the pre-close email sends with the
> switch off.
...
3. ~30 min before the close, the **pre-close drop** fires via Next's `after()` — generate the
   top-5 theses off intraday data and email everyone with `notify_preclose` set. Theses store a
...
| Endpoint | When | Does |
|---|---|---|
| `GET /api/cron/pre-close` | ~3:30 PM ET | refresh intraday gainers, generate theses, send the email drop |
